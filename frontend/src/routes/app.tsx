...
export const Route = createFileRoute("/app")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "A calm contactless communication interface prototype for ICU and paralysis patients.",
      },
...
type ScanItem = { label: string; value?: string; kind?: "letter" | "suggestion" | "undo" | "speak" };
...
function PatientView({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setScreen("yesno"), 3400);
  ...
  return (
    <div className="flex min-h-svh flex-col px-5 pb-16 pt-24 md:px-10">
      {screen !== "camera" && screen !== "calibration" && screen !== "confirmed" && (
        <nav className="mx-auto mb-6 flex w-full max-w-6xl items-center justify-between" aria-label="Communication modes">
          <p className="text-sm font-medium text-muted-foreground">Blink to select the highlighted choice</p>
          <div className="flex gap-2">
            <Button variant={screen === "needs" ? "secondary" : "ghost"} size="sm" onClick={() => setScreen("needs")}>
              <HeartPulse /> Needs
            </Button>
            <Button variant={screen === "keyboard" ? "secondary" : "ghost"} size="sm" onClick={() => setScreen("keyboard")}>
              <Keyboard /> Spell
            </Button>
          </div>
        </nav>
...
function CameraCheck({ onDone }: { onDone: () => void }) {
  ...
  return (
    <section className="w-full max-w-3xl animate-fade-in text-center" aria-label="Camera check">
      <p className="mb-3 text-sm font-semibold uppercase tracking-[0.18em] text-primary">Step 1 · Diagnosis check</p>
      ...
      <div className="relative mx-auto mt-8 aspect-video w-full overflow-hidden rounded-lg border-2 border-border bg-[#0d1424]">
        ...
        {cameraOn && (
          <div className="absolute bottom-3 left-3 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wide">
            <span className="rounded-md bg-signal px-2 py-1 text-signal-foreground">Face detected</span>
      ...
      <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
        ...
        <Button size="lg" onClick={() => { stopCamera(); onDone(); }}>
...
function Calibration() {
  return (
    <section className="animate-fade-in text-center" aria-label="Calibrating blink detection">
      ...
      <h1 className="font-display text-3xl font-semibold md:text-5xl">Getting to know your blink...</h1>
      <p className="mx-auto mt-5 max-w-lg text-lg text-muted-foreground">Keep your eyes relaxed. There is nothing you need to do.</p>
...
function YesNo({ onDone }: { onDone: () => void }) {
  ...
  useSpaceSelect(() => choose(index));

  return (
    <section className="w-full max-w-5xl animate-fade-in text-center">
      <p className="mb-3 text-sm font-semibold uppercase tracking-[0.18em] text-primary">Let’s start simply</p>
      <h1 className="font-display text-3xl font-semibold md:text-5xl">Can you see both choices clearly?</h1>
      <div className="mt-10 grid gap-5 sm:grid-cols-2">
        {["Yes", "No"].map((label, itemIndex) => (
          <ScanButton key={label} active={index === itemIndex} selected={selected === itemIndex} onClick={() => { setIndex(itemIndex); choose(itemIndex); }}>
...
function NeedsBoard({ onSelect }: { onSelect: (value: string) => void }) {
  const { index, setIndex } = useScanner(NEEDS.length);
  const [selected, setSelected] = useState<number | null>(null);
  const choose = useCallback(
    (choice: number) => {
      if (selected !== null) return;
      setSelected(choice);
      window.setTimeout(() => onSelect(NEEDS[choice]?.label ?? ""), 850);
    },
    [onSelect, selected],
  );
  useSpaceSelect(() => choose(index));
  ...
  return (
    <section className="w-full max-w-6xl animate-fade-in">
      <div className="mb-7 text-center">
        <p className="mb-2 text-sm font-semibold uppercase tracking-[0.18em] text-primary">Quick needs</p>
        <h1 className="font-display text-3xl font-semibold md:text-5xl">What would you like to say?</h1>
      </div>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
        {NEEDS.map((item, itemIndex) => (
          <ScanButton
            key={item.label}
            ...
            onClick={() => { setIndex(itemIndex); choose(itemIndex); }}
          >
...
function ScanButton({ active, selected, compact, tone, children, onClick }: { active: boolean; selected: boolean; compact?: boolean; tone?: string; children: React.ReactNode; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function ScanningKeyboard({ message, setMessage, onSpeak }: { message: string; setMessage: React.Dispatch<React.SetStateAction<string>>; onSpeak: (value: string) => void }) {
  const scanItems = useMemo<ScanItem[]>(
    () => [...SUGGESTIONS, ...LETTERS, { label: "Space", value: " ", kind: "letter" }, { label: "Undo", kind: "undo" }, { label: "Speak", kind: "speak" }],
    [],
  );
  ...
  const choose = useCallback(
    (choice: number) => {
      ...
      else if (item.kind === "speak") onSpeak(message);
  ...
  return (
    <section className="w-full max-w-6xl animate-fade-in py-2">
      <p className="mb-2 text-center text-sm font-semibold uppercase tracking-[0.18em] text-primary">Compose a message</p>
      <div className="mb-4 min-h-24 rounded-lg border border-border bg-card p-5 shadow-sm md:min-h-28 md:p-7">
        <p className={cn("font-display text-2xl font-medium md:text-4xl", !message && "text-muted-foreground")}>
          {message || "Your message will appear here"}<span className="ml-1 inline-block h-8 w-0.5 bg-primary align-middle motion-safe:animate-pulse" />
        </p>
      </div>
      <div className="mb-4 grid gap-2 md:grid-cols-3">
        {SUGGESTIONS.map((item, suggestionIndex) => (
          <KeyboardKey key={item.label} label={item.label} active={index === suggestionIndex} selected={selected === suggestionIndex} wide onClick={() => { setIndex(suggestionIndex); choose(suggestionIndex); }} />
        ))}
      </div>
      <div className="grid grid-cols-7 gap-1.5 sm:grid-cols-10 md:grid-cols-10 md:gap-2">
        {LETTERS.map((item, letterIndex) => {
          ...
          return <KeyboardKey key={item.label} label={item.label} active={index === itemIndex} selected={selected === itemIndex} onClick={() => { setIndex(itemIndex); choose(itemIndex); }} />;
        })}
        {scanItems.slice(-3).map((item, offset) => {
          ...
          return (
            <KeyboardKey
              key={item.label}
              label={item.label}
              icon={item.kind === "undo" ? <Undo2 /> : item.kind === "speak" ? <Volume2 /> : undefined}
              active={index === itemIndex}
              selected={selected === itemIndex}
              wide
              onClick={() => { setIndex(itemIndex); choose(itemIndex); }}
...
function KeyboardKey({ label, active, selected, wide, icon, onClick }: { label: string; active: boolean; selected: boolean; wide?: boolean; icon?: React.ReactNode; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function Confirmed({ message, onAgain }: { message: string; onAgain: () => void }) {
  return (
    <section className="w-full max-w-4xl animate-fade-in text-center">
      <div className="mx-auto mb-8 grid size-16 place-items-center rounded-full bg-success text-success-foreground"><Check className="size-8" /></div>
      <p className="text-sm font-semibold uppercase tracking-[0.18em] text-success">Message confirmed</p>
      <h1 className="mx-auto mt-5 max-w-3xl font-display text-4xl font-semibold leading-tight md:text-7xl">“{message}”</h1>
      <div className="mx-auto mt-10 flex w-fit items-center gap-5 rounded-lg border border-border bg-card px-8 py-5">
        ...
        <div className="sound-wave flex h-9 items-center gap-1" aria-label="Speaking">
          {[0, 1, 2, 3, 4].map((bar) => <span key={bar} style={{ animationDelay: `${bar * 0.12}s` }} />)}
        </div>
        <span className="font-medium">Speaking</span>
      </div>
      <Button size="lg" variant="secondary" className="mt-10" onClick={onAgain}><RotateCcw /> New message</Button>
    </section>
  );
}

function useSpaceSelect(onSelect: () => void) {
  const actionRef = useRef(onSelect);
  actionRef.current = onSelect;
  useEffect(() => {
